    ...
    CalendarApp.prototype.onEventClick =  function (calEvent, jsEvent, view) {
          ...
          var $this = this;
              ...
              success: function(data){
                $.each(data, function(i, item) {
                  ...
                  form.find(".row")
                      ...
                      .append("<div class='col-md-7'><div class='form-group'><label class='control-label'>Titulo</label><input required class='form-control' id='titulo' name='titulo'  type='text'  value='" + calEvent.title + "' /></div></div>")
                      .append("<div class='col-md-7'><div class='form-group'><label class='control-label'>Descripcion</label><textarea required class='form-control' id='descripcion' name='descripcion'>"+item.descripcion+"</textarea></div></div>")
                      .append("<div class='col-md-3'><div class='form-group'><label class='control-label'>Valor</label><input required class='form-control' id='precio' name='precio'  type='number' step='0.01'  value='"+item.costo+"' /></div></div>")
                      .append("<div class='col-md-4'>"+
                        "<label class='control-label'>Tipo de Evento</label>"+
                        "<select class='form-control form-white' data-placeholder='Choose a color...'' name='tipo' id='tipo' required>"+
                      ...
                      .append("<div class='col-md-7'>"+
                              "<label class='control-label'>Genero Musical</label>"+
                              "<select id='select-music' class='select2 select2-multiple' multiple='multiple' required data-placeholder='Seleccione Genero Musical' name='event-music[]'>"+
                              item.gn+
                              "</select></div>")
                      .append("<div class='col-md-7'>"+
                            "<label class='control-label'>Tipo de musica</label>"+
                            "<select id='select-infraestructura' class='form-control form-white' data-placeholder='Choose a cr...' required name='event-infraestructura'></select></div>")
            ...
            $this.$modal.modal({
              ...
              var ids = document.getElementById('idEvent').value;
              ...
              $.ajax({
                type: "GET",
                url: "/web_service_gn/"+calEvent.id,
                success: function(response){
                  $('#select-infraestructura').html(response.infraestructura);
                }
              });
            ...
            $this.$modal.find('form').on('submit', function () {
                ...
                data.append('descripcion', CKEDITOR.instances['descripcion'].getData());
    ...
    CalendarApp.prototype.onSelect = function (start, end, allDay) {
        var $this = this;
            $this.$modal.modal({
              ...
              $.ajax({
                type: "GET",
                url: "/select_calendario",
                success: function(response){
                  $('#select-infraestructura').html(response.infraestructura);
                }
              });
              CKEDITOR.replace('descripcion',{height : 150,});
              $(".select2").select2();
            ...
            $.ajax({
              type: "GET",
              url: "/select_genero_musical",
              success: function(data){
                $.each(data, function(i, item) {
                  form.append("<div class='row'></div>");
                  form.find(".row")
                      .append("<div class='col-md-5'><label class='control-label'>Imagen</label><img id='output' class='img-responsive img-thumbnail' src='https://i.pinimg.com/736x/ed/3e/66/ed3e660d4d40886cd87f6edcd5a5c7e1.jpg' alt=''><br><br><input type='file' id='portada' class='form-control form-white' name='portada' accept='image/*' onchange='loadFile(event)' placeholder='Sube una Imagen' required></div>")
                      .append("<div class='col-md-7'><div class='form-group'><label class='control-label'>Titulo</label><input required class='form-control' id='titulo' name='event-titulo'  type='text'  value='' placeholder='Titulo'/></div></div>")
                      .append("<div class='col-md-7'><div class='form-group'><label class='control-label'>Descripcion</label><textarea required class='form-control' id='descripcion' name='event-descripcion'></textarea></div></div>")
                      .append("<div class='col-md-7'><div class='form-group'><label class='control-label'>Valor</label><input required class='form-control' id='precio' name='event-valor'  type='number' step='0.01'  value='' placeholder='$ 0.00' /></div></div>")
                      .append("<div class='col-md-3'>"+
                        "<label class='control-label'>Tipo de Evento</label>"+
                        "<select class='form-control form-white' data-placeholder='Choose a color...'' name='event-tipo' id='tipo' required>"+
                          "<option value='0'>Seleccionar</option>"+
                          "<option value='1'>Formal</option>"+
                          "<option value='2'>SemiFomal</option>"+
                          "<option value='3'>Informal</option>"+
            ...
            $this.$modal.find('form').on('submit', function () {
                var data = new FormData(this);
                data.append('event-descripcion', CKEDITOR.instances['descripcion'].getData());
